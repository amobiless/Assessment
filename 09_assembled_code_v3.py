...
class Results:
    def __init__(self):
        ...
        self.entry_box = Entry(self.results_frame)
        self.entry_box.grid(row=2, column=0, padx=5, pady=10)
    ...
    def participant(self):
        ...
        error = "Invalid entry. Must be a name followed by a " \
                "time greater than 0"
        result.strip()
        result = result.split()
        ...
        elif self.string_checker(result[1][0]):
            time = float(result[1])
            if time < 1:
                messagebox.showerror("Error", error)
        ...
        # Checks the number of appended items in the list
        # and gives error if 0
        if len(self.result_list) == 0:
            error = "The list of participants is empty. " \
    ...
    def help(self):
        get_help = Help(self)
        get_help.help_text.configure(text="In the first text box,"
                                          " please enter the length of "
                                          "the race in meters.\n"
                                          "In the second text box, please "
                                          "enter the name and time of the "
                                          "participant.\n"
...
class History:
    def __init__(self, partner, result_list):
        ...
        self.race_length = partner.entry_box.get()
        ...
        if len(result_list) >= max_length:
            for index in range(len(result_list) - 1,
                               (len(result_list) - max_length) - 1, -1):
                history_string += result_list[index][1].title() + " : " + \
                                  str(result_list[index][0]) + "\n"
        else:
        ...
        self.export_button = Button(self.export_dismiss_frame, text="Export",
                                    font="arial 12 bold",
                                    command=lambda: self.export(result_list))
...
class Export:
    def __init__(self, partner, result_list):
        ...
        # If users press cross at top, closes export dialogue
        ...
        self.export_text = Label(self.export_frame,
                                 text="If filename already exists "
                                      "it will be replaced.",
                                 justify=LEFT,
                                 bg="#ffafaf", fg="maroon",
                                 font="Arial 10 italic",
    ...
    def save_history(self, partner, result_list):
        ...
        if has_error == "yes":
            ...
            f.write(f"{partner.race_length}m race\n")
            for element in result_list:
                f.write(f"{(result_list.index(element) + 1)} "
                        f"Name: {element[1].title()} - "
                        f"Time: {element[0]} seconds\n")
